...
def evaluate_metrics(scores, matches, metrics, cross_validation=True, return_roc=False, roc_name_path=None):
    stats = {}
    acc = 0
    if cross_validation:
    ...
    for metric in metrics:
        if metric == 'eer':
            ...
            stats['acc'] = np.mean(acc)
        elif metric == 'acc_std':
            # stats[metric] = np.std(acc * 100)
            stats['acc_std'] = np.std(acc)
    
    if roc_name_path is not None:
        with open(roc_name_path, 'w') as fw:

...
class TinyFaceTest:
    ...
    def test_identification(self, features, ranks=[1,5,20], similarity="cosine", model_name=None):
        feat_probe = features[self.indices_probe]
        feat_gallery = features[self.indices_gallery]
        compare_func = lambda x, y: identification_similarity(x, y, sim_method=similarity, model_name=model_name)

...
if __name__ == '__main__':
    ...
    # set save root
    gpu_id = args.gpu
    save_path = os.path.join('./tinyface_result', args.loss_model, "fusion_{}".format(args.fusion_method))
    ...
    dataloader = data_utils.prepare_dataloader(img_paths,  args.batch_size, num_workers=args.num_workers)
    features, norms = infer(model, dataloader, use_flip_test=args.use_flip_test, fusion_method=args.fusion_method)
    
    features = features * norms
    
    results = tinyface_test.test_identification(features, ranks=[1,5,10], similarity=args.similarity, model_name=args.loss_model)
    print(results)
    pd.DataFrame({'rank':[1,5,10], 'values':results}).to_csv(os.path.join(save_path, 'result.csv'))
